...
* [Secure Spring Boot backend and protect Vue.js frontend](#secure-spring-boot-backend-and-protect-vuejs-frontend)
  * [Secure the backend API with Spring Security](#secure-the-backend-de.florian.moehle.api-with-spring-security)
...
Prior to Vue 2.0, there was a build in solution (vue-resource). But from 2.0 on, 3rd party libraries are necessary. One of them is [Axios](https://github.com/mzabriskie/axios) - also see blog post https://alligator.io/vuejs/rest-de.florian.moehle.api-axios/
...
callRestService ();{
  axios.get(`de.florian.moehle.api/hello`)
    .then(response => {
...
public WebMvcConfigurer corsConfigurer() {
  return new WebMvcConfigurerAdapter() {
    ...
    public void addCorsMappings(CorsRegistry registry) {
      registry.addMapping("/de.florian.moehle.api/*").allowedOrigins("http://localhost:8080");
    }
  };
}
```

Now all calls to resources behind `de.florian.moehle.api/` will return the correct CORS headers. 
...
```js
module.exports = {
  // proxy all webpack dev-server requests starting with /de.florian.moehle.api
  // to our Spring Boot backend (localhost:8098) using http-proxy-middleware
  // see https://cli.vuejs.org/config/#devserver-proxy
  devServer: {
    proxy: {
      '/de.florian.moehle.api': {
...
```js
// proxy de.florian.moehle.api requests
...
```
    // Forwards all routes to FrontEnd except: '/', '/index.html', '/de.florian.moehle.api', '/de.florian.moehle.api/**'
    // Required because of 'mode: history' usage in frontend routing, see README for further details
    @RequestMapping(value = "{_:^(?!index\\.html|de.florian.moehle.api).$}")
...
This controller will forward every request other then `'/', '/index.html', '/de.florian.moehle.api', '/de.florian.moehle.api/**'` to our Vue.js frontend.
...
2019-01-29 09:42:07.621  INFO 8 --- [           main] d.j.s.Application         : Starting Application v0.0.1-SNAPSHOT on 745e854d7781 with PID 8 (/app.jar started by root in /)
2019-01-29 09:42:07.627  INFO 8 --- [           main] d.j.s.Application         : No active profile set, falling back to default profiles: default
...
2019-01-29 09:42:17.745  INFO 8 --- [           main] d.j.s.Application         : Started Application in 10.823 seconds (JVM running for 11.485)
```
...
2019-11-02 16:15:37.967  INFO 7 --- [           main] d.j.s.Application         : Starting Application v0.0.1-SNAPSHOT on aa490bc6ddf4 with PID 7 (/app.jar started by root in /)
2019-11-02 16:15:37.973  INFO 7 --- [           main] d.j.s.Application         : No active profile set, falling back to default profiles: default
...
Now let's focus on securing our Spring Boot backend first! Therefore we introduce a new RESTful resource, that we want to secure specifically:
                   ...
                   +---+                  +---+                  +---+
                   |   | /de.florian.moehle.api/hello       |   | /de.florian.moehle.api/user        |   | /de.florian.moehle.api/secured
                   +---+                  +---+                  +---+
                     |                      |                      |
        +-----------------------------------------------------------------------+
        |                                                                       |
...
public class WebSecurityConfiguration extends WebSecurityConfigurerAdapter {
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        .and()
            .authorizeRequests()
                .antMatchers("/de.florian.moehle.api/hello").permitAll()
                .antMatchers("/de.florian.moehle.api/user/**").permitAll() // allow every URI, that begins with '/de.florian.moehle.api/user/'
                .antMatchers("/de.florian.moehle.api/secured").authenticated()
...
```
	...
	public void secured_api_should_react_with_unauthorized_per_default() {
		...
		given()
		.when()
			.get("/de.florian.moehle.api/secured")
		.then()
...
The simplest solution one could think about how to secure our frontend, would be to create a simple Login.vue component, that simply accesses the `/de.florian.moehle.api/secured` resource every time the login is used.
...
<script>
import de.florian.moehle.api from './backend-de.florian.moehle.api'
...
,
  methods: {
    callLogin() {
      de.florian.moehle.api.getSecured(this.user, this.password).then(response => {
...
With this simple implementation, the Login component asks the Spring Boot backend, if a user is allowed to access the `/de.florian.moehle.api/secured` resource. The [backend-de.florian.moehle.api.js](frontend/src/components/backend-de.florian.moehle.api.js) provides an method, which uses axios' Basic Auth feature:
...
```
    ...
    actions: {
        async login({commit}, user, password) {
            de.florian.moehle.api.getSecured(user, password)
                .then(response => {
                    console.log("Response: '" + response.data + "' with Statuscode " + response.status);
                    if(response.status == 200) {
...
export default {
  ...
  methods: {
    getSecuredTextFromBackend() {
      de.florian.moehle.api.getSecured(store.getters.getUserName, store.getters.getUserPass)
              .then(response => {
                console.log("Response: '" + response.data + "' with Statuscode " + response.status);
